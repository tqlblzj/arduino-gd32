...
void PBKDF2_HMACBuilder::clearData() {
  if (derivedKey != nullptr) {
    force_memzero(derivedKey, derivedKeyLen);
    delete[] derivedKey;
    derivedKey = nullptr;
  }
  derivedKeyLen = 0;
...
void PBKDF2_HMACBuilder::hmac(const uint8_t *key, size_t keyLen, const uint8_t *data, size_t dataLen, uint8_t *output) {
  ...
  if (keyLen > HMAC_BLOCK_SIZE) {
    ...
    // Copy key to keyPad
    sys_memcpy(keyPad, key, keyLen);
  }

  // Pad key with zeros if necessary
  if (keyLen < HMAC_BLOCK_SIZE) {
    sys_memset(keyPad + keyLen, 0, HMAC_BLOCK_SIZE - keyLen);
  }
...
void PBKDF2_HMACBuilder::add(const uint8_t *data, size_t len) {
  printf("PBKDF2_HMACBuilder::add sets only the password. Use setPassword() and setSalt() instead.\r\n");
  setPassword(data, len);
}

bool PBKDF2_HMACBuilder::addStream(Stream &stream, const size_t maxLen) {
  printf("PBKDF2_HMACBuilder does not support addStream. Use setPassword() and setSalt() instead.\r\n");
  (void)stream;
  (void)maxLen;
  return false;
}
...
void PBKDF2_HMACBuilder::calculate() {
  if (password == nullptr || salt == nullptr) {
    printf("Error: Password or salt not set.\r\n");
    return;
  }
...
void PBKDF2_HMACBuilder::getBytes(uint8_t *output) {
  if (!calculated || derivedKey == nullptr) {
    printf("Error: PBKDF2-HMAC not calculated or no output buffer provided.\r\n");
    return;
  }
  sys_memcpy(output, derivedKey, derivedKeyLen);
}
...
void PBKDF2_HMACBuilder::setPassword(const uint8_t *password, size_t len) {
  if (this->password != nullptr) {
    force_memzero(this->password, len);
    delete[] this->password;
  }
  this->password = new uint8_t[len];
  sys_memcpy(this->password, password, len);
...
void PBKDF2_HMACBuilder::setSalt(const uint8_t *salt, size_t len) {
  if (this->salt != nullptr) {
    force_memzero(this->salt, len);
    delete[] this->salt;
  }
  this->salt = new uint8_t[len];
  sys_memcpy(this->salt, salt, len);
  this->saltLen = len;
  calculated = false;
}
...
void PBKDF2_HMACBuilder::pbkdf2_hmac(
  ...
  for (size_t i = 1; i <= blocks; i++) {
    // Prepare salt || INT(i)
    sys_memcpy(saltWithBlock, salt, saltLen);
    ...
    sys_memcpy(block, u1, hashSize);

    // U2 = HMAC(password, U1)
    for (uint32_t j = 1; j < iterations; j++) {
      hmac(password, passwordLen, u1, hashSize, u2);
      sys_memcpy(u1, u2, hashSize);
    ...
    sys_memcpy(output + (i - 1) * hashSize, block, copyLen);
  }
}
