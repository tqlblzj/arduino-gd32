...
void SHA1Builder::add(const uint8_t *data, size_t len) {
  ...
  if (left && len >= fill) {
    sys_memcpy((void *)(buffer + left), data, fill);
    process(buffer);
    data += fill;
    len -= fill;
    left = 0;
  ...
  if (len > 0) {
    sys_memcpy((void *)(buffer + left), data, len);
  }
}
...
void SHA1Builder::getBytes(uint8_t *output) {
  sys_memcpy(output, hash, SHA1_HASH_SIZE);
}

void SHA1Builder::getChars(char *output) {
  if (!finalized || output == nullptr) {
    printf("Error: SHA1 not calculated or no output buffer provided.\r\n");
    return;
  }
