...
void SHA2Builder::begin() {
  ...
  sys_memset(state_32, 0, sizeof(state_32));
  sys_memset(state_64, 0, sizeof(state_64));
...
void SHA2Builder::add(const uint8_t *data, size_t len) {
  ...
  if (buffer_size > 0) {
    ...
    sys_memcpy(buffer + buffer_size, data, to_copy);
    buffer_size += to_copy;
    offset += to_copy;
  ...
  if (offset < len) {
    sys_memcpy(buffer, data + offset, len - offset);
...
void SHA2Builder::getBytes(uint8_t *output) {
  sys_memcpy(output, hash, hash_size);
}

// Get the hash as hex string
void SHA2Builder::getChars(char *output) {
  if (!finalized || output == nullptr) {
    printf("Error: SHA2 not calculated or no output buffer provided.\r\n");
    return;
  }
