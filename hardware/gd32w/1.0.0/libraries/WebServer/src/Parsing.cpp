...
// Function to get HTTP method string from enum
const char *http_method_str(HTTPMethod method) {
  if (method == HTTP_ANY) {
    return "ANY";
  }
  int num_methods = sizeof(_http_method_str) / sizeof(const char *);
  if ((int)method >= 0 && (int)method < num_methods) {
    return _http_method_str[(int)method];
  }
  return "UNKNOWN";
}
...
static char *readBytesWithTimeout(NetworkClient &client, size_t maxLength, size_t &dataLength, int timeout_ms) {
  ...
  while (dataLength < maxLength) {
    ...
    if (!buf) {
      buf = (char *)sys_malloc(newLength + 1);
      if (!buf) {
        return nullptr;
      }
    } else {
...
bool WebServer::_parseForm(NetworkClient &client, const String &boundary, uint32_t len) {
  (void)len;
  printf("Parse Form: Boundary: %s Length: %u\r\n", boundary.c_str(), len);
  String line;
  int retry = 0;
  do {
      ...
      if (line.length() > (size_t)19 && line.substring(0, 19).equalsIgnoreCase(F("Content-Disposition"))) {
        ...
        if (nameStart != -1) {
          ...
          if (!argIsFile) {
            ...
            _currentUpload->needAuthBeforeUpload = false;
            ...
            // If needing auth during upload handler, abort processing
            if (_currentUpload->needAuthBeforeUpload) {
              printf("needAuthBeforeUpload!!!\r\n");
            ...
            printf("End File: %s Type: %s Size: %d\r\n", _currentUpload->filename.c_str(), _currentUpload->type.c_str(), (int)_currentUpload->totalSize);
            if (!client.connected()) {
              return _parseFormUploadAborted();
            }
            line = client.readStringUntil('\r');
