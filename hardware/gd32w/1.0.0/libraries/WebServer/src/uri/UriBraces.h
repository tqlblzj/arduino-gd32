...
public:
  ...
  // to see how many {} pairs are in the uri and resize pathArgs accordingly!!!
  // for example, for "/userID/{}/userName/{}", pathArgs will be resized to 2
  ...
  bool canHandle(const String &requestUri, std::vector<String> &pathArgs) override final {
    // if the URI doesn't contain any {} pairs, we can just compare the URI with the request URI
    if (Uri::canHandle(requestUri, pathArgs)) {
      return true;
    }

    // otherwise, we need to parse the URI and extract the path arguments.
    // for example, if the URI is "/userID/{}/userName/{}", and the request
    // URI is "/userID/123/userName/john", we need to extract "123" and "john" as path arguments
