...
void NetworkServer::begin(uint16_t port, int enable) {
  ...
  if (_addr.type() == IPv4) {
    sys_memcpy(server.sin6_addr.s6_addr + 11, (uint8_t *)&_addr[0], 4);
    server.sin6_addr.s6_addr[10] = 0xFF;
    server.sin6_addr.s6_addr[11] = 0xFF;
  } else {
    sys_memcpy(server.sin6_addr.s6_addr, (uint8_t *)&_addr[0], 16);
  }
  ...
  setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int));
  sys_memcpy((uint8_t *)&(server.sin_addr.s_addr), (uint8_t *)&_addr[0], 4);
