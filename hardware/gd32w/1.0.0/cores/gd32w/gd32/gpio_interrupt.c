...
void gpio_interrupt_enable(uint32_t portNum, uint32_t pinNum, void (*callback)(void), uint32_t mode)
{
    ...
    gpio_mode_set(gpio_port[portNum], GPIO_MODE_INPUT, GPIO_OSPEED_25MHZ, gpio_pin[pinNum]);
    eclic_irq_enable(gpio_exti_infor[pinNum].irqNum, 9, 0);
...
void gpio_interrupt_disable(uint32_t pinNum)
{
    ...
    eclic_irq_disable(gpio_exti_infor[pinNum].irqNum);
}
