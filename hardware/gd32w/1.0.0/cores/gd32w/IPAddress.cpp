...
IPAddress::IPAddress(uint32_t address) {
    ...
    // based on network byte order (will be value on big endian systems),
    // e.g. http://2398766798 is same as http://142.250.70.206,
    // However on little endian systems, octets 0x83, 0xFA, 0x46, 0xCE,
    // in that order, will form an integer (uint32_t) 3460758158 .
}
...
String IPAddress::toString(bool includeZone) const {
    ...
    if (_type == IPv4) {
        char buf[16];
        sprintf(buf, "%u.%u.%u.%u",
                _address.bytes[IPADDRESS_V4_BYTES_INDEX],
                _address.bytes[IPADDRESS_V4_BYTES_INDEX + 1],
                _address.bytes[IPADDRESS_V4_BYTES_INDEX + 2],
        ...
        for (int f = 0; f < 8; f++) {
            if (f < longest_start || f >= longest_start + longest_length) {
                uint16_t val = (_address.bytes[f * 2] << 8) | _address.bytes[f * 2 + 1];
                pos += sprintf(buf + pos, "%x", val);
                if (f < 7) {
                    buf[pos++] = ':';
                }
...
bool IPAddress::fromString6(const char *address) {
    ...
    while (*address) {
        ...
        if (isalnum(c) && c <= 'f') {
            ...
            // Parse zone suffix as a zone number
...
size_t IPAddress::printTo(Print &p, bool includeZone) const {
    ...
    if (_type == IPv6) {
        ...
        // Add zone if zone-id is non-zero
